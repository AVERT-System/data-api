# -*- coding: utf-8 -*-
"""
Handlers for CRUD operators and other actions for the endpoints defined in the infrared
imagery API.
...
def create(image: Image) -> int:
    """
    Handler for POST requests to the '/api/admin/infrared/c' endpoint.
    ...
    r = requests.post(
        url="http://localhost:8081/api/admin/imagery/infrared/c", json=image
    )
    ```
...
def retrieve_image(image_id: str, download=None) -> any:
    """
    Handler for GET requests to the '/api/imagery/infrared/r/{image_id}' endpoint.
    ...
    kwargs = (
        {"download_name": f"{image_id}.jpg", "as_attachment": True} if download else {}
    )

    return send_from_directory(
        directory=(
            f"{app.config['DATA_ARCHIVE_PATH']}/imagery"
            f"/infrared/{vnum}/{year}/{site}/{julday}"
        ),
        path=f"{image_id}.jpg",
        mimetype="image/jpeg",
        **kwargs,
...
def query(**kwargs) -> Response:
    """
    Handler for GET requests to the '/api/imagery/infrared/query' endpoint.
...
def delete(image_id: str) -> int:
    """
    Handler for DELETE requests to the '/api/admin/imagery/infrared/d/{image_id}'
    endpoint.
    ...
    r = requests.delete(
        url=f"http://localhost:8081/api/admin/imageryinfrared/d/{image_id}"
    )
    ```
